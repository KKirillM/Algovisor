...
type
  ...
  TMTEStructureExProc = function (Idx: Integer; Version: Integer;var Msg: PMTEMsg): TMTEResult; {$ifdef Unix}cdecl{$else}stdcall{$endif};
...
type
  ...
  private
    ...
    fMTEStructureEx: TMTEStructureExProc;
  ...
  public
    ...
    property MTEStructureEx: TMTEStructureExProc read fMTEStructureEx;
    // helper function
...
function MTEStructureEx(Idx: Integer; Version: Integer; var Msg: PMTEMsg): TMTEResult; {$ifdef Unix}cdecl{$else}stdcall{$endif};
...
const
  sFunctions: array [0..28] of PAnsiChar = (
    'MTEConnect',
    'MTEDisconnect',
    'MTEStructure',
    'MTEExecTrans',
...
begin
  ...
  if Assigned(fMTEStructureEx) then Include(fMTESrlAddons, msaStructureEx);
end;
...
var
  MTESrlAddons: TMTESrlAddOns = [
    ...
    msaStructure2, msaExecTransEx, msaGetTablesFromSnapshot,
    msaOpenTableAtSnapshot, msaStructureEx
  ];
...
function MTEStructureEx; external micexdll;
...
var
  ...
  _MTEStructureEx: TMTEStructureExProc = nil;
...
resourcestring
  ...
  rsNoStructureEx = 'MTEStructureEx() function is not supported by MTESRL.DLL library';
...
begin
  ...
  begin
    ...
    begin
      ...
      _MTEStructureEx := GetProcAddress(H, 'MTEStructureEx');
      ...
      if Assigned(_MTEStructureEx) then Include(MTESrlAddons, msaStructureEx);
    end;
  end;
end;
...
function MTEStructureEx(Idx: Integer; Version: Integer; var Msg: PMTEMsg): TMTEResult;
begin
  LoadMTESrl;
  if Assigned(_MTEStructureEx) then
    Result := _MTEStructureEx(Idx, Version, Msg)
  else
  begin
    Result := MTE_NOTIMPLEMENTED;
    if @Msg <> nil then begin
      PInteger(@TmpBuf)^ := Length(rsNoStructureEx);
      StrLCopy(PAnsiChar(@TmpBuf) + SizeOf(Integer), PAnsiChar(AnsiString(rsNoStructureEx)), SizeOf(TmpBuf) - SizeOf(Integer));
      Msg := @TmpBuf;
    end;
  end;
end;
